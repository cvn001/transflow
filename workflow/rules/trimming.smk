...
rule qc_pe:
    ...
    threads:
        SAMPLE_THREADS
    shell:
        "trimmomatic PE -quiet -threads {threads} -phred33 {input.notcomb1} {input.notcomb2} {output.p1} {output.up1} {output.p2} {output.up2} SLIDINGWINDOW:4:15 MINLEN:{READ_MIN_LEN}"
...
rule qc_se:
    ...
    threads:
        SAMPLE_THREADS
    shell:
        "trimmomatic SE -quiet -threads {threads} -phred33 {input.extfraq} {output} SLIDINGWINDOW:4:{MIN_QUAL} MINLEN:{READ_MIN_LEN}"
...
rule prepare_mapping:
    ...
    output:
        fq1 = temp("temp/ready/{smp}/{smp}.pe_1.fastq.gz"),
        fq2 = temp("temp/ready/{smp}/{smp}.pe_2.fastq.gz"),
        fqm = temp("temp/ready/{smp}/{smp}.se.fastq.gz")
    shell:
        """
